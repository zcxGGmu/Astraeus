...
export function extractBrowserOperation(toolName: string | undefined): string {
  ...
  let operation = toolName
    .replace(/^browser[_-]/, '')  // 移除 browser_ 或 browser- 前缀
  ...
  // 使用映射表或默认格式化
  operation = operationMap[operation.toLowerCase()] || 
...
export function getToolComponent(toolName: string): string {
  ...
  switch (normalizedName) {
    ...
    case 'delete-tasks':
    case 'get-tasks':
      return 'GenericToolView'; // 使用通用视图，现在已修复解析问题

    // Test tools
    case 'test-calculator':
    case 'test-echo':
      return 'GenericToolView'; // 使用通用视图
